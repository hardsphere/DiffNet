...
def sumdR2( Ris, K):
    ddR2 = matrix( 0., ((K*(K+1))>>1, (K*(K+1))>>1))
    for i in range(K):
        Ri = Ris[i]
        # dR[:(a,b)] = R[:,a] - R[:,b]
        dR = matrix( 0., (K, (K*(K+1))>>1))
        pairwise_diff( Ri[:K,:K], dR, K)
        # ddR[:(ap,bp)] = dR'[:,ap] - dR'[:,bp]
        ddR = matrix( 0., ((K*(K+1))>>1, (K*(K+1))>>1))
...
def Aopt_KKT_solver( si2, W):
    ...
    si2ab = matrix( 0., ((K*(K+1))>>1, 1))
    p = 0
    for i in range(K):
        si2ab[p:p+(K-i)] = si2[i:,i]
        p += (K-i)

    si2q = matrix( 0., ((K*(K+1))>>1, (K*(K+1))>>1))
    blas.syr( si2ab, si2q)
    
    sRVR = cvxopt.mul( si2q, ddR2)

    #  We first solve for K(K+1)/2 n_{ab}, K u_i, 1 y
    nvars = int(K*(K+1)//2) + K # + 1  We solve y by elimination of n and u.
    ...
    Bm[:int(K*(K+1)//2),:int(K*(K+1)//2)] = cvxopt.mul( si2q, ddR2)
    ...
    Bm[int(K*(K+1)//2):int(K*(K+1)//2)+K,:-K] = dg2
    ...
    def kkt_solver( x, y, z):
        ...
        x[:int(K*(K+1)//2)] -= l[:int(K*(K+1)//2)]

        dL = matrix( 0., (int(K*(K+1)//2), 1))
        ab = 0
        for a in range(K):
            dL[ab] = Ls[a,a]
            ab += 1
        ...
        x[:int(K*(K+1)//2)] -= cvxopt.mul( si2ab, dL)

        # The RHS of equations
        # g_i^t F g_i + R_{i,K+1,K+1}^2 u_i = pi - L_{i,K+1,K+1}
        x[int(K*(K+1)//2):] -= l[int(K*(K+1)//2)+(K+1)*(K+1)-1::(K+1)*(K+1)]
        ...
        z[:int(K*(K+1)//2)] += cvxopt.mul( dis, x[:int(K*(K+1)//2)])
        z[:int(K*(K+1)//2)] *= -1.
...
def Aopt_GhA( si2, nsofar=None, di2=None, G_as_function=False):
    ...
    if G_as_function:
        ...
        nrows, ncols = int(K*(K+1)//2) + K*(K+1)*(K+1), int(K*(K+1)//2) + K
    ...
    # h vector.
    h = matrix( 0., (int(K*(K+1)//2) + K*(K+1)*(K+1), 1))
    # F := Fisher matrix
    if nsofar is None:
        F = matrix( 0., (K, K))
    else:
    ...
    # A matrix
    A = matrix( np.concatenate( [ np.ones( int(K*(K+1)//2)), np.zeros( K) ]), 
                (1, int(K*(K+1)//2) + K))
...
def A_optimize_fast( sij, N=1., nsofar=None, delta=None, 
    ...
    cv = matrix( np.concatenate( [ np.zeros( int(K*(K+1)//2)), np.ones( K) ]),
                 (int(K*(K+1)//2) + K, 1))
