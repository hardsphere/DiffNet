...
def sparse_A_optimal_network( sij, nadd=1., nsofar=None, 
    ...
    if not sparse_by_fluctuation:
        ...
        nij = A_optimize( matrix(sij), nadd, nsofar=matrix(nsofar))
    ...
    for i in range(K):
        edges.append( (-1, i, weight( i,i)))
        for j in range(i+1, K):
            edges.append( (i, j, weight(i,j)))
            
    edges = list(nx.k_edge_augmentation( G, k=connectivity, avail=edges,
                                         partial=True))
    ...
    # TODO: update update_A_optimal_sdp for relative-only networks.
    # nij = update_A_optimal_sdp( sij, nadd, nsofar, only_include_measurements)
    nij = A_optimize( matrix(sij), nadd, matrix(nsofar),
                      only_include_measurements=only_include_measurements)
    return nij
